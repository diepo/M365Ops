...
- Get-M365OpsMailboxArchiveDetail {Identity} - scorciatoia comoda che combina in UNA chiamata ArchiveName/ArchiveStatus/ArchiveState/ArchiveGuid (da Get-Mailbox -Archive) CON dimensione/item/ultimo logon dell'archivio (da Get-MailboxStatistics -Archive) - usa questa quando la domanda riguarda l'archivio nel suo complesso; usa Get-M365OpsMailboxDetail/Get-M365OpsMailboxStatistics separatamente solo se sai gia' esattamente quale dei due tagli di dati (identita' vs statistiche) ti serve. Il campo 'ArchiveEnabled' del risultato e' gia' calcolato correttamente da ArchiveGuid (mai da ArchiveStatus, inaffidabile - vedi la nota su Get-M365OpsMailboxDetail) - fidati di quel campo, non ricalcolarlo tu stesso guardando ArchiveStatus.
